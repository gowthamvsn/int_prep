...
- **The predictive maintenance pattern itself** is one I've built before, just in a different domain: the Pneumonia Detection and Alzheimer's MRI classifiers are both "predict a costly negative outcome early enough to act on it" problems with the same asymmetric-cost structure (missing a real case is far worse than a false alarm) that locomotive component-failure prediction has. The modeling mechanics — transfer learning on limited labeled data, choosing recall-oriented thresholds, needing interpretability (Grad-CAM/SHAP) so a domain expert trusts the flag — transfer directly, even though X-rays and vibration sensors are different sensors entirely.
